...
public class MysticalController extends BukkitRunnable {
    ...
    private static final AttributeModifier HEALTH_MODIFIER = new AttributeModifier(
            HEALTH_MODIFIER_UUID, "HealthBonus", 20.0, AttributeModifier.Operation.ADD_NUMBER);

    @Override
    public void run() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (ConfigData.getDay() >= 30 && wearingUpgradedArmor(player)) {
                updateHealth(player);
                player.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 50, 1));
            } else {
    ...
    public void applyHealthModifier(Player player) {
        ...
        if (!hasHealthModifier(player)) {
            ...
            player.setHealth(currentHealth + 20); // Ajustar la salud actual al modificar el atributo
        }
    }
